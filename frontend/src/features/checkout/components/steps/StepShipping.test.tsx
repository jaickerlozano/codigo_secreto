...
const santiagoOptions: DispatchOptions = { comunaId: 1, mode: 'santiago', dates: ['2026-08-25', '2026-08-27'], shippingOption: null }
const regionalOptions: DispatchOptions = { comunaId: 3, mode: 'regional', dates: null, shippingOption: { shippingOptionId: 7, key: 'chilexpress', carrier: 'Chilexpress', tariff: 4900, minLeadDays: 2, maxLeadDays: 4 } }

function renderShipping(overrides: Partial<Parameters<typeof StepShipping>[0]> = {}) {
  const props = { comunaId: 1, destinationName: 'Santiago', destinationRegion: 'Región Metropolitana', shippingCost: 3500, quoteIsLoading: false, quoteIsError: false, quoteError: null, onRetryQuote: vi.fn(), selection: {}, onSubmit: vi.fn(), onBack: vi.fn(), ...overrides }
...
describe('StepShipping explicit dispatch selection', () => {
  it('renders backend Santiago dates and blocks continue until one is chosen', async () => {
    ...
    expect(await screen.findByRole('radio', { name: /25 de agosto/ })).toBeDefined()
  ...
  it('submits the explicit Santiago standard selection', async () => {
    ...
    expect(onSubmit).toHaveBeenCalledWith({ deliveryKind: 'standard', requestedDispatchDate: '2026-08-25' })
  })
  ...
  it('offers a labeled special date and submits it as special delivery', async () => {
    ...
    await user.click(await screen.findByRole('radio', { name: 'Solicitar otra fecha' }))
    fireEvent.change(screen.getByLabelText('Fecha deseada'), { target: { value: '2026-09-01' } })

    expect(isDisabled(continueButton())).toBe(false)
    await user.click(continueButton())

    expect(onSubmit).toHaveBeenCalledWith({ deliveryKind: 'special', requestedDispatchDate: '2026-09-01' })
  })
  ...
  it('requires explicit selection of the regional carrier card', async () => {
    ...
    const { user, onSubmit } = renderShipping({ comunaId: 3, destinationName: 'Viña del Mar' })
    ...
    expect(onSubmit).toHaveBeenCalledWith({ deliveryKind: 'standard', shippingOptionId: 7 })
  })

  it('blocks continue when delivery is unavailable for the comuna', async () => {
    vi.mocked(getDispatchOptions).mockResolvedValue({ comunaId: 9, mode: 'regional', dates: null, shippingOption: null })
    renderShipping({ comunaId: 9 })
  ...
  it('shows dispatch error with retry and recovers', async () => {
    vi.mocked(getDispatchOptions)
      .mockRejectedValueOnce(new Error('No pudimos cargar las opciones de envío.'))
      .mockResolvedValueOnce(santiagoOptions)
    const { user } = renderShipping()

    expect((await screen.findByRole('alert')).textContent).toContain('No pudimos cargar las opciones de envío.')
    expect(isDisabled(continueButton())).toBe(true)
  ...
  it('keeps blocking and offers quote retry when the backend quote fails', async () => {
    ...
    const { user } = renderShipping({ quoteIsError: true, quoteError: new Error('No pudimos calcular el costo de envío.'), onRetryQuote })

    expect((await screen.findByRole('alert')).textContent).toContain('No pudimos calcular el costo de envío.')
    expect(isDisabled(continueButton())).toBe(true)
  ...
  it('clears an invalidated selection and requires a new choice', async () => {
    vi.mocked(getDispatchOptions).mockResolvedValue({ comunaId: 1, mode: 'santiago', dates: ['2026-08-27', '2026-08-29'], shippingOption: null })
    renderShipping({ selection: { deliveryKind: 'standard', requestedDispatchDate: '2026-08-25' } })
