...
const radioCardClass =
  'flex min-h-12 cursor-pointer items-center gap-3 rounded-xl border border-white/10 bg-secondary px-4 transition-all has-[:checked]:border-neon-cyan/70 has-[:checked]:bg-neon-cyan/5'
...
export function StepShipping({
  ...
  const hasResolvedDestination = comunaId !== null && comunaId > 0

  const dispatch = useQuery({
    queryKey: ['dispatch-options', comunaId],
    queryFn: () => getDispatchOptions(comunaId as number),
    enabled: hasResolvedDestination,
    retry: false,
  })
  const options = dispatch.data ?? null
  const regionalOption =
    options !== null && options.mode === 'regional'
      ? options.shippingOption
      : null

  useEffect(() => {
    if (options?.mode === 'regional' && options.shippingOption === null && Object.keys(picked).length === 0) {
      setPicked({ deliveryKind: 'standard' })
    }
    if (
      options &&
      Object.keys(picked).length > 0 &&
      !isDispatchSelectionValid(picked, options)
    ) {
      setPicked({})
      setCustomMode(false)
      setCustomDate('')
  ...
  const optionsReady =
    !dispatch.isLoading && !dispatch.isError && options !== null
  const selectionReady =
    options !== null && isDispatchSelectionValid(picked, options)
  ...
  const changeCustomDate = (value: string) => {
    ...
    setInvalidated(false)
    setPicked(
      value ? { deliveryKind: 'special', requestedDispatchDate: value } : {}
    )
  }
  ...
  const invalidatedNotice = invalidated && (
    <p className="mb-2 text-xs text-neon-cyan" role="status" aria-live="polite">
      Tu selección anterior ya no está disponible. Elige nuevamente.
    </p>
  )
      ...
      <div className="mb-5 flex items-start gap-3 rounded-2xl bg-secondary p-4">
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">
          ...
          <strong className="text-foreground">
            {hasResolvedDestination ? destinationName || '—' : '—'}
          </strong>
          {hasResolvedDestination && destinationRegion
            ? `, ${destinationRegion}`
            : ''}
        </p>
      </div>
        ...
        {quoteIsError ? (
          <div className="space-y-3" role="alert">
            <p className="flex items-center gap-1.5 text-xs text-destructive">
              ...
              {quoteError?.message ?? 'No pudimos calcular el costo de envío.'}
            </p>
            <button
              type="button"
              onClick={onRetryQuote}
              className="min-h-12 w-full rounded-xl border border-neon-cyan/60 px-4 text-xs font-bold uppercase text-neon-cyan focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
              Reintentar
            </button>
          </div>
      ...
      {!hasResolvedDestination ? (
        <div className="mb-5 rounded-2xl bg-secondary p-4" role="alert">
          <p className="text-xs text-destructive">
            Selecciona una comuna en el paso anterior para ver las opciones de
            envío.
          </p>
        </div>
      ) : dispatch.isError ? (
        <div
          className="mb-5 space-y-3 rounded-2xl bg-secondary p-4"
          role="alert"
        >
          <p className="flex items-center gap-1.5 text-xs text-destructive">
            <AlertCircle size={11} aria-hidden="true" />{' '}
        ...
        <div
          className="mb-5 rounded-2xl bg-secondary p-4"
          role="status"
          aria-live="polite"
        >
        ...
        <div className="mb-5">
            ...
            {options.dates.map((date) => (
              <label key={date} className={radioCardClass}>
                <input
                  ...
                  value={date}
                  checked={
                    picked.deliveryKind === 'standard' &&
                    picked.requestedDispatchDate === date
                  }
                  onChange={() => chooseStandard(date)}
          ...
          {customMode && (
            <div className="mt-3 rounded-xl border border-neon-cyan/30 bg-secondary p-4">
              <label
                htmlFor="special-dispatch-date"
                className="mb-1 block text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground"
              >
                Fecha deseada
              </label>
              <input
                id="special-dispatch-date"
                type="date"
                min={tomorrowIso()}
                value={customDate}
                onChange={(e) => changeCustomDate(e.target.value)}
                className="min-h-12 w-full rounded-xl border border-white/10 bg-background px-3 text-sm text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              />
              <p className="mt-1 text-[10px] text-muted-foreground">
                La entrega especial requiere confirmación previa por WhatsApp.
              </p>
            </div>
          )}
        </div>
      ) : options.mode === 'regional' ? (
        <div className="mb-5">
          <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground">
            Envío regional
          </p>
          ...
          <label className={radioCardClass}>
            <input
              type="radio"
              name="dispatch-regional"
              checked={
                regionalOption
                  ? picked.shippingOptionId === regionalOption.shippingOptionId
                  : picked.deliveryKind === 'standard' &&
                    picked.shippingOptionId == null &&
                    picked.requestedDispatchDate == null
              }
              onChange={() =>
                regionalOption
                  ? chooseOption(regionalOption.shippingOptionId)
                  : chooseRegional()
              }
              className="h-4 w-4 accent-neon-cyan"
            />
            <span className="flex-1">
              <span className="flex items-center justify-between gap-2">
                <span className="text-sm font-bold text-foreground">
                  {regionalOption?.carrier ?? 'Envío regional'}
                </span>
              </span>
              <span className="text-xs text-muted-foreground">
                {regionalOption
                  ? `Entrega en ${regionalOption.minLeadDays}–${regionalOption.maxLeadDays} días hábiles · Embalaje discreto`
                  : 'Detalles de transporte se confirmarán al despachar.'}
              </span>
            </span>
          </label>
        </div>
        ...
        <div className="mb-5 rounded-2xl bg-secondary p-4" role="alert">
          <p className="flex items-center gap-1.5 text-xs text-destructive">
            <AlertCircle size={11} aria-hidden="true" /> El envío no está
            disponible para esta comuna.
          </p>
        </div>
      )}
